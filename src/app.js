(() => {
  ...
  function renderTop() {
    ...
    $('#instrState').textContent = state.instructor ? '강사 모드가 켜져 있습니다. 강사방 · 예시 답안 · 다시 시작을 쓸 수 있습니다.' : '강사 비밀번호를 넣으면 강사방 · 예시 답안 · 3-1 케이스 C 참고 답안이 열립니다.';
    ...
    app.innerHTML = `
      <header class="sheet-head">
        ...
        <div class="sheet-tools">
          ...
          ${state.instructor ? '<button type="button" class="btn ghost danger" data-act="clear">이 시트 비우기</button>' : ''}
        </div>
        ${ansNote}
  ...
  function subscribeTeam() {
    ...
    teamUnsub = DB.doc(`teams/${state.teamId}`).onSnapshot((snap) => {
      ...
      if (!snap.exists) { if (controlReady && state.mission && Object.keys(D()).length) writing = writing.then(() => writeTeam({})).catch(() => {}); return; }
      const d = snap.data();
  ...
  function applyReset() {
    ...
    if (!state.instructor) { state.teamId = ''; state.team = ''; state.author = ''; $('#team').value = ''; $('#author').value = ''; }
    state.mission = null; state.sheet.campaign = 's0'; state.imports = {};
  ...
  async function resetAll() {
    if (!DB) {
      ...
      state.mission = null; save(); render(); flash('이 브라우저의 조 기록을 지웠습니다'); return;
    }
    const at = new Date().toISOString();
    try { await DB.doc('control/room').set({ round: -1, all: false, endsAt: null, resetAt: at }); applyReset(); }
  ...
  async function setControl(next) {
    if (control?.resetAt && !next.resetAt) next = { ...next, resetAt: control.resetAt };
